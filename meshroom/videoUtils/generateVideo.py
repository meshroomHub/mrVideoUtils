...
class GenerateVideo(desc.CommandLineNode):
    ...
    def processChunk(self, chunk):
        ...
        if not ret:
            logging.error(f"Cannot open input {chunk.node.input.value}")
            raise RuntimeError()
        ...
        if len(viewsPerFrameId) == 0:
            logging.error("No valid views in SfmData")
            raise RuntimeError()
        ...
        intermediate_directory = Path(chunk.node.directory.value) / "links"
        ...
        self.commandLine = f"ffmpeg -y -framerate {chunk.node.framerate.value} -start_number 0 -i {intermediate_directory}/%012d{suffix}  -vf \"scale=trunc(iw/2)*2:trunc(ih/2)*2\" -c:v libx264 -pix_fmt yuv420p {chunk.node.outputVideo.value}"
